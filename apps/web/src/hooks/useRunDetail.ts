...
import { openRunStream } from "../services/runs/runStream";
import type { Recommendation, RecommendationRun, RunEvent, SourceDocument } from "../types/api";
import { sortRecommendationsByScore } from "../utils/recommendations";
import { getRunOutcome, type RunOutcome } from "../utils/runs";
...
export function useRunDetail({ token, runId, onRunUpdate }: UseRunDetailOptions) {
  ...
  useEffect(() => {
    ...
    const refreshFromApi = async () => {
      const data = await runService.getRun(token, runId);
      if (!active) {
        return null;
      }
      applyRun(data);
      const eventList = await runService.getRunEvents(token, runId);
      if (active) {
        setEvents(eventList);
      }
      setLoadingRun(false);
