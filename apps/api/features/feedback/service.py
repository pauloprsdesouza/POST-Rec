...
class FeedbackService:
    ...
    ) -> tuple[RecommendationFeedback, CalibrationTask | None]:
        existing = (
        ...
        if user_id and get_settings().ranking_calibration_enabled and not in_experiment:
            ...
            if feedback_count >= 5 and feedback_count % 5 == 0:
                ...
                def run_calibration() -> None:
                    ...
                    try:
                        ranking_calibration_service.calibrate(session)
                    except Exception:
                        pass
                    finally:
                        session.close()

                calibration_task = run_calibration

        return feedback, calibration_task
