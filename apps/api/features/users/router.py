...
def get_my_consent(
    ...
    data = load_cached_json(key, CacheTTL.CONSENT, load, terminal=True)
...
def get_my_account(current_user: User = Depends(get_current_user_required)):
    ...
    data = load_cached_json(key, CacheTTL.ACCOUNT, load, terminal=True)
...
def get_my_profile(
    ...
    data = load_cached_json(key, CacheTTL.PROFILE, load, terminal=True)
...
def list_my_runs(
    ...
    def load() -> list[dict]:
        return run_summaries_payload(db, current_user.id, capped)

    data = load_cached_json(key, CacheTTL.RUNS_LIST, load, terminal=True)
    return [RunSummaryResponse.model_validate(item) for item in data]
