...
class FacetVerificationService:
    ...
    def embed_corpus_facets(
        ...
        for facet in FacetType.all_ordered():
            for idx, corpus_text in enumerate(lfm.corpus_texts_for(facet)):
                if corpus_text.strip():
                    ...
                    text_keys.append(f"{facet.value}:{idx}")
        ...
        for key, emb in zip(text_keys, embeddings, strict=False):
            facet_key = key.split(":", 1)[0]
            corpus_facet_embeddings.setdefault(facet_key, []).append(emb)
        return corpus_facet_embeddings

    def batch_proposal_facet_embeddings(
        self,
        db: Session,
        run_id: str,
        ...
        for rec_index, recommendation in enumerate(recommendations):
            ...
            for facet in FacetType.all_ordered():
                ...
                if prop_text:
                    ...
                    text_keys.append((rec_index, facet.value))

        if not texts_to_embed:
            return [{} for _ in recommendations]

        embeddings = gemini_service.generate_embeddings(db, run_id, texts_to_embed)
        by_index: list[dict[str, list[float]]] = [{} for _ in recommendations]
        for (rec_index, facet_value), emb in zip(text_keys, embeddings, strict=False):
            by_index[rec_index][facet_value] = emb
        return by_index
    ...
    def verify_fggv(
        ...
        *,
        ...
        proposal_facet_embeddings: dict[str, list[float]] | None = None,
    ) -> dict[str, Any]:
        lfm = facet_map or build_literature_facet_map(papers)
        facet_embeddings = dict(proposal_facet_embeddings or {})
        corpus_embeddings = dict(corpus_facet_embeddings or {})

        if not facet_embeddings or not corpus_embeddings:
            try:
                embedded_proposal, embedded_corpus = self._embed_facets(db, run_id, recommendation, lfm)
                facet_embeddings = facet_embeddings or embedded_proposal
                corpus_embeddings = corpus_embeddings or embedded_corpus
            except Exception:
                pass
        ...
        fni, per_facet, diagnostics = compute_facet_novelty_index(
            ...
            corpus_facet_embeddings=corpus_embeddings or None,
