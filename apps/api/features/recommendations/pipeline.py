...
class SotaPipelineService:
    ...
    def _verify_all(
        ...
        *,
        ...
        from packages.postrec_core.scoring.novelty_verification import proposal_text
        ...
        if mode.uses_fggv_verification:
            lfm = facet_map or facet_verification_service.build_facet_map(papers)
            if facet_map is None:
                facet_map = lfm
            corpus_facet_embeddings = facet_verification_service.embed_corpus_facets(db, run_id, lfm)
            proposal_facet_embeddings_by_index = facet_verification_service.batch_proposal_facet_embeddings(
                db, run_id, dict_recommendations
            )

        verified: list[dict[str, Any]] = []
        for index, rec in enumerate(dict_recommendations):
            verified.append(
                novelty_verification_service.verify_recommendation(
                    ...
                    gap_matrix=gap_matrix,
                    facet_map=facet_map,
                    proposal_embedding=proposal_embeddings[index] if index < len(proposal_embeddings) else None,
                    corpus_facet_embeddings=corpus_facet_embeddings,
                    proposal_facet_embeddings=(
                        proposal_facet_embeddings_by_index[index]
                        if index < len(proposal_facet_embeddings_by_index)
                        else None
                    ),
                )
            )
