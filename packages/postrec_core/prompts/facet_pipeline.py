...
from packages.postrec_core.prompts.shared_grounding import GENERATION_COT_PHASES, GROUNDING_CONSTITUTION

FGGV_PROPOSAL_USER_TEMPLATE = f"""Research area: {{research_area}}
...
Generate up to {{max_recommendations}} research recommendations using Facet-Grounded Gap Verification (FGGV).
...
{{{{
  "recommendations": [
    {{{{
      ...
      "facet_deltas": {{{{
        "problem": "what problem facet is new vs corpus (cite paper_ids)",
...
- Return empty recommendations if insufficient evidence in the landscape or no topic-matching papers."""
